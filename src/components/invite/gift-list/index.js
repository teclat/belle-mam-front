...
export default class GiftList extends Component {
    ...
    gift = async () => {
        ...
        axios.post(Constants.ApiUrl + 'events/give-gift', {
            user_id: user.id, event_id: this.props.event.id, product_id: this.state.gift.id
        }, {
            headers: {
    ...
    render() {
        return (
            <section id="present-list" class={this.props.event.theme == "green" ? "invite-green" : "invite-purple"}>
                        ...
                        this.props.event && this.props.event.products ?
                            this.props.event.products.map((product) => {
                                return <li key={product.id} class="d-flex flex-column justify-content-around align-items-center">
                                    ...
                                    <button
                                        onClick={() => { this.setState({ showGift: true, gift: product }) }}
                                        class={"btn " + (this.props.event.theme == "green" ? 'btn-secondary' : 'btn-primary')}>DAR PRESENTE!</button>
                                </li>
                            })
                            : null
                    }
